...
def formatYelpData(jsonData):
    ...
    for x in jsonData["businesses"]:
        ...
        temp["tripDetails"] = tripDetails(originalCoord,destCoord)
...
#main view functions
def index(request):
    originalCoord = getGeoCoordinates()
    return render(request, 'exploree/index.html')


def food(request):
    r = getYelpInfo('dinner', originalCoord)
...
def activitites(request):
    ...
    '''
    Currenly returns a shuffled response, change this to return the best in a certain distance 
    or return the ones that the highest rated first then best in the radius
    '''
    # random.shuffle(r)
    # return render(request, 'exploree/activities.html', context={'raw_data':r})
    
    r = getYelpInfo('activities', originalCoord)
    if r.status_code==200:
